  ...
  <section class="space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        ...
        <h1 class="section-title mt-3">Banner Management</h1>
        <p class="muted-text mt-2">Control homepage sliders and jump links.</p>
      </div>
      <el-button type="primary" size="large" @click="openCreate">New Banner</el-button>
    </div>
    ...
    <el-card class="border-0 bg-white/80 shadow-soft">
      <div class="flex flex-wrap items-end gap-3">
        ...
        <el-button type="primary" @click="fetchBanners">Search</el-button>
      ...
      <el-table :data="banners" class="mt-6" v-loading="loading" stripe>
        ...
        <el-table-column prop="linkType" label="Link Type" width="120" />
        <el-table-column prop="linkTarget" label="Target" min-width="200" />
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="520px">
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="saving" @click="saveBanner">Save</el-button>
      </template>
    </el-dialog>
  </section>
</template>
...
const dialogTitle = computed(() => (editingId.value ? 'Edit Banner' : 'Create Banner'))
...
const fetchBanners = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      ElMessage.error(res.message || 'Failed to load banners')
    }
  } catch (error) {
...
const saveBanner = async () => {
  if (!form.imageUrl || !form.linkTarget) {
    ElMessage.warning('Image URL and target are required')
    return
  }
  saving.value = true
  try {
